...
import InfoPanelFactory from "./infobar.js";
...
export default function Neo4jD3(_selector, _options) {
  var container,
    ...
    options = {
      ...
      neo4jJsonAuthHeader: undefined, //HR: Adds auth header as option
  ...
  /*
   * HR: added since not using package manager
   */
  ...
  function loadNeo4jDataFromUrl(neo4jDataUrl, auth) {
    ...
    /*
     * HR: adds authorization header to d3.json call
     * new d3 api uses promise
     */

    d3.json(neo4jDataUrl, { headers: { Authorization: auth } }).then(function(
      data
    ) {
      //if (error) {
      //throw error;
  ...
  /*
   * HR: data returned from server is already pre-processed
   * so no need to to format from
   * for d3. Jus wrape in graph object tiht appropriate keys
   */
  ...
  //HR:  removed random data generation code
  ...
  /*
   * HR: removed methods related to generating random data
   */
  return {
