class ChatChannel < ApplicationCable::Channel
  def subscribed
    ...
    if conversation && conversation.can_participate?(current_user.neo_id)
      stream_for conversation
      Rails.logger.info "User #{current_user.neo_id} subscribed to conversation #{conversation.id}"
    else
      reject
    end
  end
  ...
  def unsubscribed
    Rails.logger.info "User #{current_user&.neo_id} unsubscribed from chat"
  end

  def send_message(data)
    conversation = Conversation.find(data['conversation_id'])
    return unless conversation&.can_participate?(current_user.neo_id)

    message = conversation.messages.create!(
      content: data['content'],
      sender_neo4j_id: current_user.neo_id,
