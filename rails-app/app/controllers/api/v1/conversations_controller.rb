module Api
  module V1
    class ConversationsController < ApplicationController
      before_action :by_context, only: %i[show_direct show_project show_topic]
      before_action :find, only: %i[show_group update destroy add_participant remove_participant]
      ...
      def create_project
        conversation = ProjectConversationService.create(params[:project_id], current_user)
      ...
      def by_context
        @conversation = ConversationService.find_by_context(params, current_user)
      rescue StandardError => e
        render json: { error: e.message }, status: :not_found
      end

      def render_conversation_with_messages
        messages_json = ConversationService.messages_with_user_permissions(@conversation, current_user.id, params)
        render json: as_json(messages_json)
      end

      def as_json(messages_json)
        {
          id: @conversation.id,
